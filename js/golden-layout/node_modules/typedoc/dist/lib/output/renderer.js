...
let Renderer = Renderer_1 = class Renderer extends component_1.ChildableComponent {
    ...
    renderDocument(page) {
        ...
        try {
            fs_1.writeFileSync(page.filename, page.contents);
        }
        catch (error) {
    ...
    prepareTheme() {
        if (!this.theme) {
            ...
            if (!fs.existsSync(path)) {
                ...
                if (!fs.existsSync(path)) {
                    this.application.logger.error(`The theme ${themeName} could not be found.`);
                    return false;
                }
            }
    ...
    async prepareOutputDirectory(directory) {
        if (fs.existsSync(directory)) {
            if (!fs.statSync(directory).isDirectory()) {
                this.application.logger.error(`The output target "${directory}" exists but it is not a directory.`);
                return false;
            }
            ...
            if (!this.theme.isOutputDirectory(directory)) {
                this.application.logger.error(`The output directory "${directory}" exists but does not seem to be a documentation generated by TypeDoc.\n` +
                    "Make sure this is the right target directory, delete the folder and rerun TypeDoc.");
                return false;
            }
            try {
                await fs_1.remove(directory);
            }
            catch (error) {
