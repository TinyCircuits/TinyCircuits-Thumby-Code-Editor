...
function writeFileSync(fileName, data) {
    fs.mkdirSync(path_1.dirname(normalizePath(fileName)), { recursive: true });
...
async function writeFile(fileName, data) {
    await fs_1.promises.mkdir(path_1.dirname(normalizePath(fileName)), {
        recursive: true,
    });
...
async function copy(src, dest) {
    ...
    if (stat.isDirectory()) {
        ...
        await Promise.all(contained.map((file) => copy(path_1.join(src, file), path_1.join(dest, file))));
    }
    else if (stat.isFile()) {
        await fs_1.promises.mkdir(path_1.dirname(dest), { recursive: true });
...
function copySync(src, dest) {
    ...
    if (stat.isDirectory()) {
        ...
        contained.forEach((file) => copySync(path_1.join(src, file), path_1.join(dest, file)));
    }
    else if (stat.isFile()) {
        fs.mkdirSync(path_1.dirname(dest), { recursive: true });
