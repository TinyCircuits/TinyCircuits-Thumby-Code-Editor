import { ComponentContainer, ComponentItemConfig, GoldenLayout, ItemType, LayoutManager, LayoutConfig } from "./golden-layout/dist/bundle/esm/golden-layout.js";
...
function registerBitmapBuilder(_container, state){
    ...
    BITMAPPER.onExport = (lines) => {
        EDITORS[parseInt(myLayout._focusedComponentItem._title.substring(6))].insert(lines);
    }
...
function registerShell(_container, state){
    ...
    ATERM.onType = (data) => REPL.writeToDevice(data);
    ...
    REPL.onFSData = (jsonStrData) => FS.updateTree(jsonStrData);
}
